...
function getUpcomingStandardPoolChars(versionStr: string): Character[] {
  ...
  return allChars.filter(
    (char) => char.version === targetVersion && !char.exclude_gacha,
  );
}
...
export default function FutureInsightPage() {
  ...
  const currentAndFuture = futureInsightData.filter(
    (item) => parseFloat(item.version) >= current,
  );
  const pastVersions = futureInsightData.filter(
    (item) => parseFloat(item.version) < current,
  );

  return (
    <div className="mx-auto max-w-5xl space-y-4 p-4 text-zinc-900 dark:text-zinc-100">
      <h1 className="mt-8 text-center text-2xl font-bold text-zinc-800 dark:text-zinc-100">
        미래시 정리
      </h1>
      <p className="mb-1 mt-0 text-center text-sm text-gray-500 dark:text-gray-400">
        향후 버전의 캐릭터 이름을 포함한 모든 번역은 의역입니다. 실제 정발
        명칭과 다를 수 있습니다.
      </p>

      {[...currentAndFuture, ...(showOldVersions ? pastVersions : [])].map(
        (item) => {
          const status = getVersionStatus(item.period.start, item.period.end);
          const upcomingStandardChars = getUpcomingStandardPoolChars(
            item.version,
          );
          ...
          return (
            ...
            >
              ...
              <div className="border-t border-zinc-200 pt-3 dark:border-zinc-700">
                <h2 className="mb-1 font-bold text-zinc-800 dark:text-zinc-100">
                  🎯 고음 카운터
                </h2>
              ...
              <div className="border-t border-zinc-200 pt-3 dark:border-zinc-700">
                <h2 className="mb-2 font-bold text-zinc-800 dark:text-zinc-100">
                  📌 픽업 배너
                </h2>
              ...
              <div className="border-t border-zinc-200 pt-3 dark:border-zinc-700">
                ...
                <p>
                  <strong>6성:</strong>{" "}
                  {item.euphoria.star6.map(getCharNameById).join(", ") || "-"}
                </p>
                <p>
                  <strong>5성:</strong>{" "}
                  {item.euphoria.star5.map(getCharNameById).join(", ") || "-"}
                </p>
              </div>
